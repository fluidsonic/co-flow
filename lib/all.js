...
/**
 * Executes all runnables and returns the result data in an array.
 * A result will be returned as soon as all passed functions completed.
 * 
 ...
 * @param  {function}          [options.unusedResultHandler=false]  Callback for each runnable whose result was not returned by the function (due to `options.failsWhenAnyFailed` or `options.failsWhenAllFailed`).
 *                                                                  The callback will be called after the function returned and in the order of the runnables.
 * 
 * @return {*[]}  Returns an array with the results of the all passed functions, in the same order.
 *                If `options.failsWhenAnyFailed` is `false` then the returned array will also contain errors for each function which failed.
 *                Returns the an empty array if `functions` evaluates to `false` or is an empty array.
 */
module.exports = function all(runnables, options) {
	...
	return function(callback) {
		...
		utility.executeRunnables(runnables, {
		...
		function onRunnableCompleted(index, error, data) {
			...
			results[index] = utility.wrapResult(error, data);
